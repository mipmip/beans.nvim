...
- [ ] 1.1 Create `lua/beans/config.lua` with the complete default table from briefing
...
- [ ] 1.2 Implement `M.setup(user)` merging via `vim.tbl_deep_extend("force",
      defaults, user or {})`; ensure list-valued leaves (e.g. `wizard.keys.finish`)
      replace rather than concatenate.
- [ ] 1.3 Implement validation: type-check known keys; for each invalid value emit at
...
- [ ] 1.4 Ensure `setup()` does not error when `beans` is absent from `$PATH` (defer
      to a `:checkhealth` warning); expose the resolved config to other modules.
- [ ] 1.5 Wire `fallback` vocab table through to `schema.lua` for use when help
      parsing fails.
- [ ] 1.6 `config_spec.lua` (§11.1): no-arg setup == full defaults; empty-table setup
...
- [ ] 2.1 Register user commands in `init.lua`: `:BeanWizard`, `:Bean` (field menu →
      value picker), `:Bean <field>` with `complete` returning field names, and the
      `:checkhealth beans` provider entry.
- [ ] 2.2 Implement `health.lua` reporting: `beans` binary + version, project root,
...
- [ ] 2.3 Implement `actions.lua` random-access entry points: use the buffer-edit
...
- [ ] 2.4 Attach buffer-local keymaps (§7.2: wizard, menu, status/type/priority,
...
- [ ] 2.5 Fire `hooks.on_attach(ctx)` after detection and `hooks.on_finish(ctx,
      changed)` after the wizard closes, with `ctx = { id, root, beans_dir, bufnr }`.
- [ ] 2.6 Verify commands and keymaps attach only in bean buffers and never in plain
      markdown (cross-checked by detect/e2e layers).

## 3. insert-completion (beans.nvim-vpcl)

- [ ] 3.1 Create `completion.lua`; set buffer-local `omnifunc` in bean buffers only.
- [ ] 3.2 Implement the context gate: complete only inside the frontmatter block after
      a known key — `status:`, `type:`, `priority:`, `parent:`, or a `tags:` list
      item; return nothing otherwise.
- [ ] 3.3 Source completion values from `schema.lua` (vocab for enums; `beans list
      --json` tag/parent candidates for tags/parent).
- [ ] 3.4 Add optional `blink.cmp` and `nvim-cmp` sources behind `completion.blink` /
      `completion.cmp` (default off), lazily `require`d; absent plugin causes no error.
- [ ] 3.5 Add NO insert-mode `<Tab>`/`<CR>` mappings anywhere.
- [ ] 3.6 Cover the context gate and optional-source no-op behaviour in specs.
